...
async function testSpecificUsers() {
  try {
    console.log(
      "🔍 Testing for posts from specific users mentioned in the issue\n"
    );
    ...
    toyKohlerPosts.forEach((post, index) => {
      ...
      console.log(
        `      Posted by: ${post.user.username} (${post.user.firstName} ${post.user.lastName})`
      );
    ...
    if (toyKohlerUser) {
      ...
      console.log(
        `   Name: ${toyKohlerUser.firstName} ${toyKohlerUser.lastName}`
      );
    ...
    if (declanConnellyUser) {
      ...
      console.log(
        `   Name: ${declanConnellyUser.firstName} ${declanConnellyUser.lastName}`
      );
      ...
      console.log(
        "❌ Declan Connelly (Tristian.VonRueden) not found in database"
      );
    }
    ...
    if (toyKohlerUser) {
      ...
      console.log(
        `   Outgoing friends: ${toyKohlerFriendships?.friends?.length || 0}`
      );
      console.log(
        `   Incoming friends: ${toyKohlerFriendships?.friendOf?.length || 0}`
      );
    }
    ...
    if (declanConnellyUser) {
      ...
      console.log(
        `   Outgoing friends: ${
          declanConnellyFriendships?.friends?.length || 0
        }`
      );
      console.log(
        `   Incoming friends: ${
          declanConnellyFriendships?.friendOf?.length || 0
        }`
      );
    }
  } catch (error) {
